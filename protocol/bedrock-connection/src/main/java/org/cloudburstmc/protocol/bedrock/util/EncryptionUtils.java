...
public class EncryptionUtils {
    ...
    public static ChainValidationResult validateDual(List<String> chain, String token)
        ...
        // Strip "MCToken " prefix if present before validating token
        String tokenJwt = token.startsWith("MCToken ") ? token.substring(8) : token;
        ChainValidationResult tokenResult;
        try {
            tokenResult = validateToken(AuthType.FULL, tokenJwt);
        } catch (Exception tokenEx) {
            tokenResult = validateToken(AuthType.SELF_SIGNED, tokenJwt);
        }
        if (chainResult == null) {
            // Chain unavailable — use the independently validated token claims.
            return new ChainValidationResult(tokenResult.signed(), tokenResult.getJwtContext());
        }
