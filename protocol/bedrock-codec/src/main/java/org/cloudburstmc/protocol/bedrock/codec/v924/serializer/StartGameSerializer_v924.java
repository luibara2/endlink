...
public class StartGameSerializer_v924 extends StartGameSerializer_v898 {
    ...
    protected void writeLevelSettings(ByteBuf buffer, BedrockCodecHelper helper, StartGamePacket packet) {
        ...
        this.writeDefaultPlayerPermission(buffer, packet.getDefaultPlayerPermission());
    ...
    protected void readLevelSettings(ByteBuf buffer, BedrockCodecHelper helper, StartGamePacket packet) {
        ...
        packet.setDefaultPlayerPermission(this.readDefaultPlayerPermission(buffer));
    ...
    /**
     * Split out so a later protocol can change the width without reimplementing the whole of
     * {@code readLevelSettings} around it: 1.26.40 narrows this from a zigzag varint to a raw int8.
     */
    protected PlayerPermission readDefaultPlayerPermission(ByteBuf buffer) {
        return PLAYER_PERMISSIONS[VarInts.readInt(buffer)];
    }

    protected void writeDefaultPlayerPermission(ByteBuf buffer, PlayerPermission permission) {
        VarInts.writeInt(buffer, permission.ordinal());
    }
