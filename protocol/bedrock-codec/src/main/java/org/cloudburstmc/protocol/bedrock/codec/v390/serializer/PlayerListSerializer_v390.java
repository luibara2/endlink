...
public class PlayerListSerializer_v390 implements BedrockPacketSerializer<PlayerListPacket> {
    ...
    public void serialize(ByteBuf buffer, BedrockCodecHelper helper, PlayerListPacket packet) {
        this.writeAction(buffer, packet.getAction());
    ...
    public void deserialize(ByteBuf buffer, BedrockCodecHelper helper, PlayerListPacket packet) {
        Action action = this.readAction(buffer);
    ...
    /**
     * Split out because 1.26.40 swaps the two ordinals: it encodes Remove as 0 and Add as 1, the
     * reverse of every earlier version.
     */
    protected Action readAction(ByteBuf buffer) {
        return Action.values()[buffer.readUnsignedByte()];
    }

    protected void writeAction(ByteBuf buffer, Action action) {
