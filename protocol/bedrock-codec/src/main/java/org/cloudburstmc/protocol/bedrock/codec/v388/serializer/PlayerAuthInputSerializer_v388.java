...
public class PlayerAuthInputSerializer_v388 implements BedrockPacketSerializer<PlayerAuthInputPacket> {
    ...
    public void serialize(ByteBuf buffer, BedrockCodecHelper helper, PlayerAuthInputPacket packet) {
        ...
        for (PlayerAuthInputData data : packet.getInputData()) {
            // See PlayerAuthInputSerializer_v748: PlayerAuthInputData outgrew 64 constants in
            // 1.26.40 and `1L << 64` wraps to bit 0, turning the newest flags into ASCEND/DESCEND.
    ...
    public void deserialize(ByteBuf buffer, BedrockCodecHelper helper, PlayerAuthInputPacket packet) {
        ...
        for (PlayerAuthInputData flag : PlayerAuthInputData.values()) {
            if (flag.ordinal() >= Long.SIZE) {
                break; // values() is ordinal-ordered; the rest cannot fit in the 64-bit word
            }
