...
public enum PersonaPieceType {
    ...
    /**
     * Like {@link #fromName(String)} but yields {@link #UNKNOWN} instead of throwing.
     *
     * <p>Local addition. Decoding somebody else's skin is not the place to be strict: Mojang adds
     * persona piece types over time, so a name this build has never seen is expected rather than
     * exceptional, and throwing would fail the enclosing skin and with it every packet that carries
     * one. Callers that need the original text back keep it themselves — see
     * {@link PersonaPieceData#getType()}.</p>
     */
    public static PersonaPieceType fromNameOrUnknown(String serializeName) {
        PersonaPieceType value = serializeNames.get(serializeName);
