...
public class BedrockCodecHelper_v712 extends BedrockCodecHelper_v575 {
    ...
    protected void writeRequestActionData(ByteBuf byteBuf, ItemStackRequestAction action) {
        if (action.getType().equals(ItemStackRequestActionType.CRAFT_RECIPE)) {
            ...
            // One byte-sized length prefix, matching readRequestActionData's
            // readArray(..., ByteBuf::readUnsignedByte, ...). Upstream wrote the byte and then called
            // the writeArray overload that prefixes an unsigned varint of its own, so a decoded
            // auto-craft re-encoded one byte too long and desynced the reader at the far end. Same
            // defect as v2168; this copy is the one v818 through v1001 inherit.
            writeArray(byteBuf, ((AutoCraftRecipeAction) action).getIngredients(),
                    ByteBuf::writeByte, this::writeIngredient);
