...
public class Bedrock_v898 extends Bedrock_v860 {
    ...
    protected static final TypeMap<CommandParam> COMMAND_PARAMS = commandParams();
    ...
    private static TypeMap<CommandParam> commandParams() {
        TypeMap.Builder<CommandParam> builder = TypeMap.builder(CommandParam.class);
        for (int i = 0; i <= 91; i++) {
            builder.insert(i, new CommandParam(i));
        }
        return builder
                .replace(0, CommandParam.UNKNOWN)
                .replace(1, CommandParam.INT)
                .replace(3, CommandParam.FLOAT)
                .replace(4, CommandParam.VALUE)
                .replace(5, CommandParam.WILDCARD_INT)
                .replace(6, CommandParam.OPERATOR)
                .replace(7, CommandParam.COMPARE_OPERATOR)
                .replace(8, CommandParam.TARGET)
                .replace(10, CommandParam.WILDCARD_TARGET)
                .replace(17, CommandParam.FILE_PATH)
                .replace(23, CommandParam.INT_RANGE)
                .replace(43, CommandParam.EQUIPMENT_SLOTS)
                .replace(44, CommandParam.STRING)
                .replace(52, CommandParam.BLOCK_POSITION)
                .replace(53, CommandParam.POSITION)
                .replace(55, CommandParam.MESSAGE)
                .replace(58, CommandParam.TEXT)
                .replace(62, CommandParam.JSON)
                .replace(71, CommandParam.BLOCK_STATES)
                .replace(74, CommandParam.COMMAND)
                .build();
    }
}
