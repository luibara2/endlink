...
public class BiomeDefinitionListSerializer_v975 extends BiomeDefinitionListSerializer_v924 {
    ...
    protected void writeDefinitionChunkGen(ByteBuf buffer, BedrockCodecHelper helper, BiomeDefinitionChunkGenData definitionChunkGen, SequencedHashSet<String> strings) {
        ...
        // readDefinitionChunkGen consumes these two, so they have to be written back.
        ...
        // Both are optional on the wire - readDefinitionChunkGen consumes a presence byte for each,
        // and a real server does send biomes with no surface builder. Writing the payload bare (as
        // upstream still does) both NPEs on those biomes and desyncs the stream by two bytes per
        // biome on the ones that survive, so the client rejects the whole biome list.
    ...
    protected void writeNoiseGradientSurface(ByteBuf buffer, BedrockCodecHelper helper, BiomeNoiseGradientSurfaceData data) {
        ...
        helper.writeArray(buffer, data.getGradientBlocks(), (buf, val) -> buf.writeIntLE(val.getBlock()));
